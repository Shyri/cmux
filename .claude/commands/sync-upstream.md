...
- **Chatmux app icon (red chevron).** The fork recolors the upstream blue chevron to magenta→red→orange in place across every `Assets.xcassets/AppIcon*.appiconset`, the two `AppIcon{Dark,Light}.imageset`, and `AppIcon.icon`. If a sync conflicts on those binary PNGs (upstream redesigned the icon) or you notice the icon went blue again, take upstream's version for the conflicted files and re-run `scripts/chatmux-recolor-icon.sh` — it's an idempotent selective hue rotation (red pixels are outside its recolor band, so re-running never double-shifts) and reapplies the Chatmux red. Requires ImageMagick (`brew install imagemagick`).
