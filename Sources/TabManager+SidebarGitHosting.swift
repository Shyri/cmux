...
extension TabManager: SidebarGitHosting {
    ...
    func hasTerminalPanel(workspaceId: UUID, panelId: UUID) -> Bool {
        ...
        // Fork extension: Claude Chat panels also benefit from sidebar git metadata
        // (branch/PR badges), so include them in the watch set the same as terminals.
        return workspace.terminalPanel(for: panelId) != nil
            || workspace.claudeChatPanel(for: panelId) != nil
    }
