...
**Never block a cooperative-pool task on an unbounded subprocess/pipe wait.** The Swift Concurrency default executor has a *fixed* width (≈ `hw.ncpu`). A synchronous block inside an `async` task — `Process.waitUntilExit()`, `DispatchGroup.wait()` with no timeout, `FileHandle.readDataToEndOfFile()` — ties up a physical pool thread, not just a logical task. Fan out enough of them and the whole pool starves: every *unrelated* `async` task app-wide stalls, including any `nonisolated async` that hops onto that pool (e.g. `ChatMcpHttpServer.start()`), while the MainActor and private `DispatchQueue`s keep running — so the app looks half-alive and the bug reads as "only *new* work hangs." Concrete incident: a `glab`/`git` helper (credential/keyring/OAuth opener) inherited fd 1/2 and outlived its parent, so `readDataToEndOfFile()` never saw EOF; the unbounded MR-approvals fan-out pinned all ~12 pool threads and new Claude Chat panels spun on "Thinking…" forever (`drainPipesInParallel` / `loadApprovals`). Rules: (1) every subprocess wait/drain carries a **hard timeout** that force-closes handles / kills the process; (2) read pipes with the throwing `read(upToCount:)` in a loop, never `readDataToEndOfFile()` (its ObjC exception on a mid-read close is uncatchable in Swift → crash); (3) **bound** any subprocess fan-out (`withTaskGroup` refill pattern, small `maxConcurrent`), never one `addTask` per item; (4) prefer moving the blocking work off the cooperative pool entirely (a private `DispatchQueue` bridged via `withCheckedContinuation`), since `Task.detached` still lands on the same pool.
